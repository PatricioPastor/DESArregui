    ...
    devtools(
        (set, get) => ({
            ...
            fetchAllOptions: async () => {
                try {
                    set({ isLoadingOptions: true });
                    const [modelsResponse, distributorsResponse] = await Promise.all([fetch("/api/models"), fetch("/api/distributors")]);

                    const [modelsResult, distributorsResult] = await Promise.all([modelsResponse.json(), distributorsResponse.json()]);
