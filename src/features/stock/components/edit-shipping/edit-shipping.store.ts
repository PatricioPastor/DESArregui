...
interface EditShippingStore {
  ...
  // Acciones para enviar
  updateShipping: () => Promise<{ success: boolean; message?: string }>;
  deliverShipping: () => Promise<{ success: boolean; message?: string }>;

  // Reset
  resetState: () => void;
}

export const useEditShippingStore = create<EditShippingStore>((set, get) => ({
  // Estado inicial
  isLoading: false,
  error: null,
  assignmentInfo: null,
  ...
  setAssignmentInfo: (info) =>
    set({ assignmentInfo: info, shippingStatus: info.shipping_status || "pending" }),
    ...
    try {
      const response = await fetch(`/api/assignments/${assignmentInfo.id}/shipping`, {
        ...
        body: JSON.stringify({
          ...
          ...(shippingNotes.trim() && { shipping_notes: shippingNotes.trim() }),
        }),
      });
  ...
  // Marcar como entregado (acción rápida)
  deliverShipping: async () => {
    const { assignmentInfo } = get();
    ...
    try {
      const response = await fetch(`/api/assignments/${assignmentInfo.id}/shipping/deliver`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      return {
        success: true,
        message: result.message || "Envío marcado como entregado exitosamente",
      };
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : "Error desconocido";
      set({ isLoading: false, error: errorMessage });
