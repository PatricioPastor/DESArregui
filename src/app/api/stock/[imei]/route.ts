...
export const GET = withAuth(async (_request: NextRequest, _session, context: { params: Promise<RouteParams> }) => {
    ...
    try {
        ...
        if (body.modelo !== undefined) {
            if (!body.modelo || typeof body.modelo !== "string") {
                return NextResponse.json({ success: false, error: "Modelo invalido" }, { status: 400 });
            }
            ...
            if (!modelRecord) {
                return NextResponse.json({ success: false, error: `Modelo "${body.modelo}" no encontrado` }, { status: 404 });
            }
        ...
        if (body.distribuidora !== undefined) {
            if (body.distribuidora && typeof body.distribuidora === "string") {
                ...
                if (!distributorRecord) {
                    return NextResponse.json({ success: false, error: `Distribuidora "${body.distribuidora}" no encontrada` }, { status: 404 });
                }
        ...
        if (body.backup_distributor_id !== undefined) {
            if (body.backup_distributor_id && typeof body.backup_distributor_id === "string") {
                ...
                if (backupDistributorRecord.name.toUpperCase() === "DEPOSITO") {
                    return NextResponse.json({ success: false, error: "DEPOSITO no puede ser una distribuidora de backup" }, { status: 400 });
                }
        ...
        if (updateData.is_backup === true && !updateData.backup_distributor_id) {
            return NextResponse.json({ success: false, error: "Distribuidora de backup es requerida cuando el dispositivo es de backup" }, { status: 400 });
        }
...
export const DELETE = withAdminOnly(async (request: NextRequest, _session, context: { params: Promise<RouteParams> }) => {
    const { imei: rawImei } = await context.params;
    const imei = rawImei?.trim();
    ...
    try {
        ...
        if (!device) {
            return NextResponse.json({ error: `No se encontró un dispositivo con IMEI ${imei}` }, { status: 404 });
        }

        // Verificar que no está ya eliminado
        if (device.is_deleted) {
            return NextResponse.json({ error: "El dispositivo ya está eliminado" }, { status: 400 });
        }

        // Verificar que no tiene asignaciones activas
        if (device.assignments.length > 0) {
            return NextResponse.json({ error: "No se puede eliminar un dispositivo con asignaciones activas" }, { status: 400 });
        }
