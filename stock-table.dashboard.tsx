  ...
  const handleSync = async () => {
    try {
      ...
      toast.success(`Sincronizaci├│n completada`, {
        description: `Creados: ${result.stats.created} ÔÇó Actualizados: ${result.stats.updated} ÔÇó Modelos: ${result.stats.createdModels} ÔÇó Distribuidoras: ${result.stats.createdDistributors} ÔÇó Errores: ${result.stats.errors}`,
        duration: 5000
      });
    ...
    <div className="space-y-6 max-h-screen ">
      ...
      <TableCard.Root>
        <TableCard.Header
          ...
          description={lastUpdated ? `├Ültima actualizaci├│n: ${formatDate(lastUpdated)}` : undefined}
          contentTrailing={
        ...
        <div className="flex flex-col gap-4 border-b border-secondary px-4 py-4 md:px-6 md:flex-row md:items-center md:justify-between">
          ...
          >
            ...
            <ButtonGroupItem id="asignados">Sin Reparaci├│n</ButtonGroupItem>
          </ButtonGroup>
        ...
        {isLoading ? (
          ...
          <>
            <Table 
              aria-label="Inventario de tel├®fonos" 
                  ...
                  <Table.Row id={item.imei}>
                    ...
                    <Table.Cell className="px-3">
                      <div className="flex justify-end gap-0.5">
                        {/* Bot├│n Asignar - Solo si es SOTI, est├í activo, en estado NEW y sin asignaci├│n */}
                        ...
                        {/* Bot├│n Ver Asignaci├│n - Solo si est├í asignado */}
                        {item.is_assigned && (
                          <ButtonUtility 
                            size="xs" 
                            color="secondary" 
                            tooltip="Ver detalles de asignaci├│n" 
                            icon={Eye}
                            onClick={() => {
                              setSelectedDevice(item);
